...
public class CleanLines {
  ...
  public static void deleteBlocks(int lineNr, Board board) {
    ...
    for (int i = 0; i < board.getFiguresOnBoard().size(); i++) {
      ...
      for (int j = 0; j < board.getFiguresOnBoard().get(i).getNrOfBlocks(); j++) {
        // all figure's blocks
    ...
    for (int i = 0; i < SIDE; i++) {
      if (cleanLines[i] != WHITE) {
        ...
        for (int j = 0; j < board.getFiguresOnBoard().size(); j++) {
          for (int j2 = 0; j2 < board.getFiguresOnBoard().get(j).getNrOfBlocks(); j2++) { // scan
                                                                                          // all
                                                                                          // blocks
            int currentYofBlock = board.getFiguresOnBoard().get(j).getBlock(j2).getPosY();
            // INCREASE Y POSITION OF EACH BLOCK HIGHER THAN DELETED LINE
            if (currentYofBlock < cleanLines[i]) {
              board.getFiguresOnBoard().get(j).getBlock(j2).setPosY((currentYofBlock + 1));
            }
          }
        }
